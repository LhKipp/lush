...
impl AstNode for ParamSignatureNode {
    fn can_cast(kind: SyntaxKind) -> bool {
        kind == SyntaxKind::ParamSignature
    }
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        if Self::can_cast(syntax.kind()) {
            Some(Self { syntax })
...
impl AstNode for VarArgParamSignatureRuleNode {
    fn can_cast(kind: SyntaxKind) -> bool {
        kind == SyntaxKind::VarArgParamSignatureRule
    }
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        if Self::can_cast(syntax.kind()) {
            Some(Self { syntax })
...
impl AstElement for LuTypeSpecifierElement {
    fn can_cast(kind: SyntaxKind) -> bool {
        match kind {
            NumberKeyword | AnyKeyword | NilKeyword | BoolKeyword | StringKeyword | FnKeyword
            | ArrayType | BareWord => true,
            _ => false,
        }
    }
    fn cast(syntax: SyntaxElement) -> Option<Self> {
        let res = match syntax.kind() {
            ...
            FnKeyword => LuTypeSpecifierElement::FnKeyword(FnKeywordToken {
                syntax: syntax.into_token().unwrap(),
            }),
